'********************************************************************
'*
'* File:           ProgramGroups.vbs 
'* Created:        March 1999
'* Version:        1.0
'*
'*  Main Function:  Enumerates the program groups on a machine.
'*
'*  ProgramGroups.vbs       [/S <server>] [/U <username>] [/W <password>] 
'*                          [/O <outputfile>]
'*
...
OPTION EXPLICIT
    ...
    Select Case intOpMode
        ...
        Case CONST_PROCEED                 
            Call LstProgramGroups(strServer     , _
                                  strOutputFile , _
                                  strUserName   , _
                                  strPassword     )
...
'********************************************************************
'*
'* Sub LstProgramGroups()
'*
'* Purpose: Enumerates the program groups on a machine.
'*
...
'*
'********************************************************************
Private Sub LstProgramGroups(strServer, strOutputFile, strUserName, strPassword)

    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService
    Dim objWshNet, objStSet, objStart
    ...
    Set objStSet = objService.InstancesOf("Win32_ProgramGroup")
    If Err.Number Then
    ...
    Call WriteLine("Program groups on Machine " & strServer, objOutputFile)
    Call WriteLine("", objOutputFile)
    
    'Generate the header
    strMessage = Empty
    strMessage = strMessage + strPackString("User",       25, 1, 1)
    strMessage = strMessage + strPackString("Group Name", 55, 1, 1)
    Call WriteLine(strMessage, objOutputFile)

    For Each objStart in objStSet
        strMessage = Empty
        strMessage = strMessage + strPackString(objStart.UserName,  25, 1, 1)
        strMessage = strMessage + strPackString(objStart.GroupName, 55, 1, 1)
        Call WriteLine(strMessage, objOutputFile)
    Next
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Enumerates the program groups on a machine."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  ProgramGroups.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>]"
    ...
    Wscript.Echo "1. cscript ProgramGroups.VBS"
    Wscript.Echo "   Lists program groups on the current machine."
    Wscript.Echo "2. cscript ProgramGroups.VBS /S MyMachine2"
    Wscript.Echo "   Lists program groups on the machine MyMachine2."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript ProgramGroups.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""ProgramGroups.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
