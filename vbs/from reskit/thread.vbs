...
'* Version:        1.0
'*
'*  Main Function:  Lists all threads currently running on a machine.
'*
'*  Thread.vbs      [/S <server>] [/U <username>] [/W <password>] 
'*                  [/O <outputfile>]
'*
...
OPTION EXPLICIT
    ...
    Select Case intOpMode
        ...
        Case CONST_PROCEED                 
            Call EnumerateThreads(strServer     , _
                                  strOutputFile , _
                                  strUserName   , _
                                  strPassword     )
...
'********************************************************************
'*
'* Sub EnumerateThreads()
'*
'* Purpose: Lists all threads currently running on a machine.
'*
...
'*
'********************************************************************
Private Sub EnumerateThreads(strServer, strOutputFile, strUserName, strPassword)

    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService
    Dim objProcInst, objEnumerator, objInst
    Dim strQuery, strMessage, strCat, strProcess
    ...
    'Get the collection of threads
    Set objEnumerator = objService.InstancesOf("Win32_Thread")
    If Err.Number Then
      Print "Error 0x" & CStr(Hex(Err.Number)) & " occurred fetching the threads."
    ...
    strCat = "Parent Process"
    strCat = strPackString(strCat, 25, 1, 1)
    strMessage = strMessage + strCat

    strCat = "Handle"
    strCat = strPackString(strCat, 10, 1, 1)
    strMessage = strMessage + strCat
    ...
    For Each objInst in objEnumerator
    strMessage = Empty

    strProcess = "Win32_Process.Handle=" + "" + objInst.ProcessHandle + ""
    set objProcInst = objService.Get(strProcess)

    strCat = objProcInst.Name
    strMessage = strMessage + strPackString(strCat, 25, 1, 1)

    strCat = objInst.Handle
    strMessage = strMessage + strPackString(strCat, 10, 1, 1)

    strCat = objInst.Priority
    strMessage = strMessage + strPackString(strCat, 10, 1, 1)

    strCat = objInst.ThreadState
    strMessage = strMessage + strPackString(strCat, 10, 1, 1)
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Lists all threads currently running on a machine."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  Thread.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>]"
    ...
    Wscript.Echo "1. Thread.vbs"
    Wscript.Echo "   lists the threads running on current machine"
    Wscript.Echo "2. Thread.vbs /S MyMachine2"
    Wscript.Echo "   lists the threads running on MyMachine2"
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript Thread.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""Thread.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
