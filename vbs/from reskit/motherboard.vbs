'********************************************************************
'*
'* File:           Motherboard.vbs 
'* Created:        March 1999
'* Version:        1.0
'*
'*  Main Function:  Get the mother board information a machine.
'*
'*  Motherboard.vbs      [/S <server>] [/U <username>] [/W <password>] 
'*                       [/O <outputfile>]
'*
...
'* Sub Motherboard()
'*
'* Purpose: Get the mother board information a machine.
'*
...
'*
'********************************************************************
Private Sub Motherboard(strServer, strOutputFile, strUserName, strPassword)

    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService, objMBOISet, objMBOI
    Dim strWBEMClass, strQuery, strMessage

    strWBEMClass = "Win32_MotherboardDevice"
  ...
  'Get the set
  Set objMBOISet = objService.InstancesOf(strWBEMClass)
  If blnErrorOccurred("Could not obtain " & strWBEMClass & " instance.") Then
    Exit Sub
  End If

  If objMBOISet.Count = 0 Then
    Call WriteLine("No mother board information is available.", objOutputFile)    
    Exit Sub
  End If
  ...
  Call WriteLine("Mother board information for Machine " & strServer, objOutputFile)
  Call WriteLine("", objOutputFile)

  For Each objMBOI In objMBOISet
    Call WriteLine("Name = " & objMBOI.Name, objOutputFile)
    Call WriteLine("Device ID = " & objMBOI.DeviceID, objOutputFile)
    Call WriteLine("Primary Bus Type = " & objMBOI.PrimaryBusType, objOutputFile)
    Call WriteLine("Secondary Bus Type = " & objMBOI.SecondaryBusType, objOutputFile)
    Call WriteLine("", objOutputFile)
  Next
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Get the mother board information a machine."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  Motherboard.vbs [/S <server>] [/U <username>] [/W <password>]"
    ...
    Wscript.Echo "1. cscript Motherboard.vbs"
    Wscript.Echo "   Get the mother board information for the current machine."
    Wscript.Echo "2. cscript Motherboard.vbs /S MyMachine2"
    Wscript.Echo "   Get the mother board information for the machine MyMachine2."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript Motherboard.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""Motherboard.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
