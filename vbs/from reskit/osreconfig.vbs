...
'* Version:        1.0
'*
'*  Main Function:  Get the OS recovery configuration for a machine
'*
'*  OsReconfig.vbs  [/S <server>] [/U <username>] [/W <password>] 
'*                   [/O <outputfile>] [/T <code>]
'*
...
OPTION EXPLICIT
    ...
    CONST CONST_LIST                    = 4
    CONST CONST_TOGGLE                  = 5

    'Declare variables
    Dim intOpMode, i
    Dim strServer, strUserName, strPassword, strOutputFile, strCode
    ...
    Select Case intOpMode
        ...
        Case CONST_LIST                 
            Call GetOSReconfig(strServer     , _
                                strOutputFile , _
                                strUserName   , _
                                strPassword     )

        Case CONST_TOGGLE                 
            Call ToggleOSConfig(strServer     , _
                                strOutputFile , _
                                strUserName   , _
                                strPassword   , _
                                strCode         )
...
'* Sub GetOSReconfig()
'*
'* Purpose: Get the OS recovery configuration for a machine.
'*
...
'*
'********************************************************************
Private Sub GetOSReconfig(strServer, strOutputFile, strUserName, strPassword)

    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService, objOsConSet, objOsCon
    Dim strQuery, strMessage, strWBEMClass

    strWBEMClass = "Win32_OSRecoveryConfiguration"
    ...
    'Get the set
    Set objOsConSet = objService.InstancesOf(strWBEMClass)
    If blnErrorOccurred("Could not obtain " & strWBEMClass & " instance.") Then
        Exit Sub
    End If

    If objOsConSet.Count = 0 Then
        Call WriteLine("No OS recovery information is available.", _
                        objOutputFile)    
        Exit Sub
    End If

    Call WriteLine("OS recovery information for Machine " & _
                   strServer, objOutputFile)
    Call WriteLine("", objOutputFile)

    For Each objOsCon In objOsConSet
        Call WriteLine("Name                          = " & _
             ParseNameFirst(objOsCon.Name), objOutputFile)
        Call WriteLine("Boot Path                     = " & _
             ParseNameSecond(objOsCon.Name), objOutputFile)
        Call WriteLine("Boot Partition                = " & _
             ParseNameThird(objOsCon.Name), objOutputFile)
        Call WriteLine("Debug File Path               = " & _
             objOsCon.DebugFilePath, objOutputFile)
        Call WriteLine("AutoReboot                    = " & _
             objOsCon.AutoReboot, objOutputFile)
        Call WriteLine("Kernel Dump Only              = " & _
             objOsCon.KernelDumpOnly, objOutputFile)
        Call WriteLine("Overwrite Existing Debug File = " & _
             objOsCon.OverwriteExistingDebugFile, objOutputFile)
        Call WriteLine("Send Admin Alert              = " & _
             objOsCon.SendAdminAlert, objOutputFile)
        Call WriteLine("Write Debug Info              = " & _
             objOsCon.WriteDebugInfo, objOutputFile)
        Call WriteLine("Write To System Log           = " & _
             objOsCon.WriteToSystemLog, objOutputFile)
        Call WriteLine("", objOutputFile)
    Next
...
'* Sub ToggleOSConfig()
'*
'* Purpose: Toggle various boolean OS recovery settings.
'*
'* Input:   strServer           a machine name
'*          strOutputFile       an output file name
'*          strUserName         the current user's name
'*          strPassword         the current user's password
'*          strCode             one of the following 2 letter codes that toggles
...
'*                               WD       WriteDebugInfo
'*                               SA       SendAdminAlert
'*                               OD       OverwriteExistingDebugFile
'*                               KD       KernelDumpOnly
'*                               AR       AutoReboot
'*
'* Output:  Modifies the revovery state.
'*
'********************************************************************
Private Sub ToggleOSConfig( ByVal strServer,      _
    ...
    If IsEmpty(strCode) Then
        Call Wscript.Echo ("Invalid code. Check your input and try again.")
        Exit Sub
    End If
    ...
    If objOsConSet.Count = 0 Then
        Call WriteLine("No OS recovery information is available.", objOutputFile)    
        Exit Sub
    End If
    ...
    i = 0
    For Each objInst In objOsConSet
        i = i + 1
        If i = 1 Then Set objOsCon = objInst
    Next
    ...
    Select Case strCode
        Case "ws"
            objOsCon.WriteToSystemLog           = NOT objOsCon.WriteToSystemLog
        Case "wd"
            objOsCon.WriteDebugInfo             = NOT objOsCon.WriteDebugInfo
        Case "sa"
            objOsCon.SendAdminAlert             = NOT objOsCon.SendAdminAlert
        Case "od"
            objOsCon.OverwriteExistingDebugFile = NOT _
                     objOsCon.OverwriteExistingDebugFile
        Case "kd"
            objOsCon.KernelDumpOnly             = NOT objOsCon.KernelDumpOnly
        Case "ar"
            objOsCon.AutoReboot                 = NOT objOsCon.AutoReboot
        Case Else
            Wscript.Echo "Invalid Code: Configuration not changed."
            objOutputFile.Close
            Exit Sub
    End Select

    Call objOsCon.Put_()
    Call WriteLine("OS recovery configuration has been changed.", objOutputFile)  
...
Private Function intParseCmdLine( ByRef strServer,        _
    ...
    Do While intArgIter <= Wscript.arguments.Count - 1
        Select Case Left(LCase(Wscript.arguments.Item(intArgIter)),2)
            ...
            Case "/t"
                intParseCmdLine = CONST_TOGGLE
                If Not blnGetArg("Code", strCode, intArgIter) Then
                    intParseCmdLine = CONST_ERROR
                    Exit Function
                End If
                intArgIter = intArgIter + 1
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Get or toggle the OS recovery configuration for a machine."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  OsReconfig.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>] [/T <code>]"
    Wscript.Echo "  [/O <outputfile>]"
    Wscript.Echo ""
    Wscript.Echo "PARAMETER SPECIFIERS:"
    Wscript.Echo "   code          A two letter code signifying " _
               & "which option to toggle."
    Wscript.Echo "                 The possible codes are list below."
    Wscript.Echo ""
    Wscript.Echo "                 Code      Description"
    Wscript.Echo "                 WS         Write To System Log"
    Wscript.Echo "                 WD         Write Debug Info"
    Wscript.Echo "                 SA         Send Admin Alert"
    Wscript.Echo "                 OD         Overwrite Existing Debug File"
    Wscript.Echo "                 KD         Kernel Dump Only"
    Wscript.Echo "                 AR         Auto Reboot"
    Wscript.Echo ""
    ...
    Wscript.Echo "1. cscript OsReconfig.vbs"
    Wscript.Echo "   Get the OS recovery configuration for the current machine."
    Wscript.Echo "2. cscript OsReconfig.vbs /S MyMachine2 /T ws"
    Wscript.Echo "   Toggle the ""Write To System Log"" option " _
               & "for the machine MyMachine2."
...
'********************************************************************
'* 
'* Function ParseNameFirst()
'*
'* Purpose: Parses the Name Property into the first line
'*
'* Input:   strName		The Property to be parsed
'*
'* Output:  The First Line of Name property data
'* 
'********************************************************************

Private Function ParseNameFirst(strName)

    Dim CurrentLetter
    CurrentLetter = 1
    Do Until Mid(strName,CurrentLetter,1) = "|" and Mid(strName,CurrentLetter+2,2) = ":\"
        CurrentLetter = CurrentLetter + 1
    Loop
    ParseNameFirst = Left(strName,CurrentLetter - 1)

End Function

'********************************************************************
'* 
'* Function ParseNameSecond()
'*
'* Purpose: Parses the Name Property into the second line
'*
'* Input:   strName		The Property to be parsed
'*
'* Output:  The Second Line of Name property data
'* 
'********************************************************************

Private Function ParseNameSecond(strName)
    Dim TempString
    Dim EndLetter

    TempString = ParseNameFirst(strName)
    TempString = Right(strName,Len(strName)-(Len(TempString)+1))
    EndLetter = 1
    Do until Mid(TempString,EndLetter,2) = "|\"
        EndLetter=EndLetter + 1
    Loop

    ParseNameSecond = Left(TempString,EndLetter - 1)

End Function

'********************************************************************
'* 
'* Function ParseNameThird()
'*
'* Purpose: Parses the Name Property into the second line
'*
'* Input:   strName		The Property to be parsed
'*
'* Output:  The Third Line of Name property data
'* 
'********************************************************************

Private Function ParseNameThird(strName)

    ParseNameThird = Right(strName,(Len(strName)-(Len(ParseNameFirst(strName)) + Len(ParseNameSecond(strName))+2)))
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript OSReconfig.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""OSReconfig.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
