'********************************************************************
'*
'* File:           EventLogMon.vbs 
'* Created:        March 1999
'* Version:        1.0
'*
'*  Main Function:  Outputs Information on CDRomDrives.
'*
'*  EventLogMon.vbs  [/S <server>] [/U <username>] [/W <password>] 
'*                   [/O <outputfile>]
'*
...
Private Sub EventLogMon(strServer     , _
                        strOutputFile , _
                        strUserName   , _
                        strPassword     )
    ...
    Dim strQuery, strTemp, strMessage
    Dim i
    ...
    ReDim strProperties(4)

    strProperties(0) = "RecordNumber"
    strProperties(1) = "LogFile"
    strProperties(2) = "SourceName"
    strProperties(3) = "TimeGenerated"
    strProperties(4) = "Message"

    Set objEnumerator = objService.ExecNotificationQuery(strQuery)
    If Err.Number Then
    ...
    For i = 0 To UBound(strProperties)
        strMessage = strMessage & strPackString(strProperties(i), 15, 1, 0)
    Next
    WriteLine strMessage, objOutputFile

    i = 0
    Do
        Set objINstance = objEnumerator.NextEvent
        If Err.Number Then
            Err.Clear
        Else
            strMessage = ""
            For i = 0 To UBound(strProperties)
                strTemp = _
                 objInstance.TargetInstance.Properties_(strProperties(i))
                strMessage = strMessage & strPackString(strTemp, 15, 1, 0)
            Next
            WriteLine strMessage, objOutputFile
        End If
        i = i + 1
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Monitors event log events."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  EventLogMon.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>]"
    ...
    Wscript.Echo "1. cscript EventLogMon.vbs"
    Wscript.Echo "   Monitors event log activity for the current machine."
    Wscript.Echo "2. cscript EventLogMon.vbs /S MyMachine2"
    Wscript.Echo "   Monitors event log activity for the machine MyMachine2."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript EventLogMon.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""EventLogMon.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
