...
'* Version:        1.0
'*
'*  Main Function:  Enables DHCP protocol on a machine. 
'*
'* 1. EnableDhcp.vbs /A adapter [/S server] [/U username] [/W password]
'*                  [/O <outputfile>]
'*
'* 2. EnableDhcp.vbs /L [/S server] [/U username] [/W password]
'*                  [/O <outputfile>]
'*
...
'*
'* Sub EnableDHCP()
'* Purpose: Enables DHCP protocol.
'* Input:   
'*          strServer       a machine name
'*          strUserName     the current user's name
'*          strPassword     the current user's password
'*          strOutputFile   the outfile file
'*          strServiceName  a network adapter servicename
'*          blnList         The option to list the adapter names
'* Output:  Results are printed on screen.
'*
'********************************************************************
Private Sub EnableDHCP(strServer     ,  _
                       strUserName   ,  _
                       strPassword   ,  _
                       strOutputFile ,  _
                       strServiceName,  _
    ...
    Dim objService, objEnumerator, objInstance, objOutputFile
    Dim intStatus
    ...
    Else
        strQuery = "Select * From Win32_NetworkAdapterConfiguration " & _
                   "Where ServiceName = """ & strServiceName & """"
    End If
    ...
    If blnList Then
        For Each objInstance in objEnumerator
            Call WriteLine(objInstance.ServiceName, objOutputFile)
        Next
    Else
        i = 0 
        For Each objInstance in objEnumerator
            intStatus = objInstance.EnableDHCP
            If Err.Number Then
                Call WriteLine("Failed to enable DHCP for " _
                              & strServiceName & ".", objOutputFile)  
            Else
                Call WriteLine("Succeeded in enabling DHCP for " _
                              & strServiceName & ".", objOutputFile)  
            End If
            i = i + 1
        Next
        ...
        If i = 0 Then
            WScript.Echo strServiceName & " is not a network adapter" _
                                        & " servicename."
        End If
    End If
...
Private Function intParseCmdLine( ByRef strServer     ,  _
    ...
    Do While intArgIter <= Wscript.arguments.Count - 1
        Select Case Left(LCase(Wscript.arguments.Item(intArgIter)),2)
            ...
            Case "/a"
                intParseCmdLine = CONST_PROCEED
                If Not blnGetArg("Adapter Service Name", strServiceName, _
                                  intArgIter) Then
                    intParseCmdLine = CONST_ERROR
                    Exit Function
                End IF
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Enables DHCP protocol on a machine."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "1. EnableDhcp.vbs /A adapter [/S server] [/U username]" _
                &" [/W password]"
    Wscript.Echo "                 [/O <outputfile>]"
    Wscript.Echo ""
    Wscript.echo "2. EnableDhcp.vbs /L [/S server] [/U username] [/W password]"
    Wscript.Echo "                 [/O <outputfile>]"
    Wscript.Echo ""
    Wscript.Echo "PARAMETER SPECIFIERS:"
    Wscript.Echo "   adapter       A network adapter service name."
    ...
    Wscript.Echo "   /L            Lists the service names of all" _
                &" network adapters."
    Wscript.Echo ""
    Wscript.Echo "EXAMPLE:"
    Wscript.Echo "1. cscript EnableDhcp.vbs /A DCX32 /S MyMachine2"
    Wscript.Echo "    Enables DHCP for adapter DCX32 on MyMachine2."
    Wscript.Echo "2. cscript EnableDhcp.vbs /L"
    Wscript.Echo "    Lists all service names on the local machine."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript EnableDHCP.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""EnableDHCP.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
