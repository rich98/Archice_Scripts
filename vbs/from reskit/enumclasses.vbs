'********************************************************************
'*
'* File:           EnumClasses.VBS 
'* Created:        March 1999
'* Version:        1.0
'*
'*  Main Function:  Enumerates WBEM classes within a namespace on a server.
'*
'*  EnumClasses.vbs [/N namespace] [C class] [/R]
'*                  [/S <server>] [/U <username>]
...
'* Output:  Results of the search are either printed on screen or
'*          saved in strOutputFile.
'*
'********************************************************************

Private Sub EnumClasses(strServer     ,  _
                        strUserName   ,  _
                        strPassword   ,  _
                        strOutputFile ,  _
                        strNameSpace  ,  _
    ...
    Call WriteLine("Classes for " & strClass & " on machine " & strServer & ":", objOutputFile)
    Call WriteLine("", objOutputFile)

    Call EnumClassRecursive(objService, strClass, 0, objOutputFile, _
                            blnRecursive)
...
Private Sub EnumClassRecursive(objService, ByVal strClass, intLevel, _
    objOutputFile, blnRecursive)

    ON ERROR RESUME NEXT

    Dim objClass, strMessage

    For Each objClass in objService.SubclassesOf(strClass, 1)
        strMessage = objClass.Path_.Class
        WriteLine Space(6*intLevel) & strMessage, objOutputFile
        If Err.Number Then
        ...
        If blnRecursive Then
            Call EnumClassRecursive(objService, strMessage, intLevel+1, _
                 objOutputFile, blnRecursive)
        End If
    Next
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Enumerates WBEM classes within a namespace on a server."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  EnumClasses.vbs [/N namespace] [C class] [/R]"
    ...
    Wscript.Echo "1. cscript EnumClasses.vbs /c __SystemClass"
    Wscript.Echo "     Lists all subclasses of __SystemClass within root\cimv2."
    Wscript.Echo "2. cscript EnumClasses.vbs /c __SystemClass /s MyMachine2"
    Wscript.Echo "     Lists all subclasses of __SystemClass for " _
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript EnumClasses.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""EnumClasses.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
