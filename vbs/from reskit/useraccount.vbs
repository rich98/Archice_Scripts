'********************************************************************
'*
'* File:           UserAccount.vbs
'* Created:        April 1999
'* Version:        1.0
'*
'*  Main Function:  Displays User Account Information.
'*
'*      UserAccount.vbs [/D <domain>] [/N <username>] </L> </B>
...
Private Function intParseCmdLine( ByRef strServer,        _
    ...
    Do While intArgIter <= Wscript.arguments.Count - 1
        Select Case Left(LCase(Wscript.arguments.Item(intArgIter)),2)
            ...
            Case "/n"
                blnQuery = True
                If Not blnGetArg("User Name", strName, intArgIter) Then
                    intParseCmdLine = CONST_ERROR
                    Exit Function
                End If
                intArgIter = intArgIter + 1
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Displays User Account Information."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  UserAccount.vbs [/D <domain>] [/N <username>] </L> </B>"
    Wscript.Echo "  [/S <server>] [/U <username>] [/W <password>]"
    Wscript.Echo "  [/O <outputfile>]"
    Wscript.Echo ""
    Wscript.Echo "PARAMETER SPECIFIERS:"
    Wscript.Echo "   domain        Filter by domain name"
    Wscript.Echo "   username      Filter by user name"
    Wscript.Echo "   /L            Filter by locked out accounts"
    Wscript.Echo "   /B            Filter by disabled accounts"
    ...
    Wscript.Echo "1. cscript UserAccount.vbs"
    Wscript.Echo "   Get the user account configurations for the" _
               & " current domain."
    Wscript.Echo "2. cscript UserAccount.vbs /d MyDomain /b"
    Wscript.Echo "   Get the disabled user account configurations " _
               & "for domain MyDomain"
    Wscript.Echo "NOTES:"
    Wscript.Echo "   This script may seem to run slow when running " _
               & "on large networks"
    Wscript.Echo "   and WANS.  If necessary use one of the filter switches."
    Wscript.Echo ""
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript UserAccount.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""UserAccount.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
