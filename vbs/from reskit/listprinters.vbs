...
'*
'*  ListPrinters.vbs      [/S <server>] [/U <username>] [/W <password>] 
'*                        [/O <outputfile>]
'*
...
'*
'********************************************************************
Private Sub ListPrinters(strServer, strOutputFile, strUserName, strPassword)


    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService, objPrinterSet, objPrinter
    ...
    Call WriteLine("Printers for machine " & _
                    strServer, objOutputFile)
    Call WriteLine("", objOutputFile)

    For Each objPrinter In objPrinterSet
        Call WriteLine("Name        = " & objPrinter.Caption, objOutputFile)
        Call WriteLine("Location    = " & objPrinter.Location, objOutputFile)
        Call WriteLine("Description = " & objPrinter.Description, objOutputFile)
        Call WriteLine("Driver      = " & objPrinter.DriverName, objOutputFile)
        Call WriteLine("Error State = " & objPrinter.DetectedErrorState, _
                        objOutputFile)
        For I = 1 to len(objPrinter.Attributes)
            If I = 1 then
                Call WriteLine("Attributes  = " & strPrinterAttributes _
                                (Mid(objPrinter.Attributes,I,1)), objOutputFile)
            Else
                Call WriteLine("              " & strPrinterAttributes _
                                (Mid(objPrinter.Attributes,I,1)), objOutputFile)            
            End If
        Next
    Next
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Lists properties of all printers installed on a computer."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  ListOs.vbs [/S <server>] [/U <username>] [/W <password>]"
    ...
    Wscript.Echo "1. cscript ListPrinters.vbs"
    Wscript.Echo "   List printer information for the current machine."
    Wscript.Echo "2. cscript ListPrinters.vbs /S MyMachine2"
    Wscript.Echo "   List printer information for the machine MyMachine2."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript ListPrinters.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""ListPrinters.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
