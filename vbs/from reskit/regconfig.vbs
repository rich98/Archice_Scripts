...
'* Version:        1.0
'*
'*  Main Function:  Outputs or modifies the registry configuration.
'*
'*  RegConfig.vbs      [/S <server>] [/U <username>] [/W <password>] 
'*                     [/O <outputfile>] [/P <proposedsize>]
'*
...
OPTION EXPLICIT
    ...
    Select Case intOpMode
        ...
        Case CONST_LIST                 
            Call LstRegConfig(strServer     , _
                              strOutputFile , _
                              strUserName   , _
                              strPassword     )

        Case CONST_PSIZE                 
            Call ChngPropSize(strServer     , _
                              strOutputFile , _
                              strUserName   , _
                              strPassword   , _
                              intSize         )
...
'* Sub LstRegConfig()
'*
'* Purpose: Outputs or modifies the registry configuration.
'*
...
'*
'********************************************************************
Private Sub LstRegConfig(strServer, strOutputFile, strUserName, strPassword)

    ON ERROR RESUME NEXT

    Dim objFileSystem, objOutputFile, objService
    Dim objRegSet, objReg, objInst, objWshNet
    ...
    'Get the first registry instance
    Set objRegSet = objService.InstancesOf("Win32_Registry")
    If Err.Number Then
        Wscript.Echo("Error 0x" & CStr(Hex(Err.Number)) & _
                     " occurred obtaining the program groups.")
    ...
    i = 0
    For Each objInst In objRegSet
        i = i + 1
        If i = 1 Then Set objReg = objInst
    Next
    ...
    Call WriteLine("Registry configuration on Machine " & _
                    strServer, objOutputFile)
    Call Writeline("Registry name = " & objReg.Name, objOutputFile)
    Call WriteLine("", objOutputFile)

    'Generate the header
    strMessage = Empty
    strMessage = strMessage + strPackString("Current Size (MB)", 25, 1, 1)
    strMessage = strMessage + strPackString("Maximum Size (MB)", 25, 1, 1)
    strMessage = strMessage + strPackString("Proposed Size (MB)",25, 1, 1)
    Call WriteLine(strMessage, objOutputFile)

    'Output the registry information
    strMessage = Empty
    strMessage = strMessage + strPackString(objReg.CurrentSize,  25, 1, 1)
    strMessage = strMessage + strPackString(objReg.MaximumSize,  25, 1, 1)
    strMessage = strMessage + strPackString(objReg.ProposedSize, 25, 1, 1)
...
'*          strUserName         the current user's name
'*          strPassword         the current user's password
'*          intSize             the proposed size of the registry
'*
'* Output:  Results are either printed on screen or saved in strOutputFile.
'*
'********************************************************************
Private Sub ChngPropSize( strServer,      _
                          strOutputFile,  _ 
                          strUserName,    _
                          strPassword,    _
                          intSize)
    ...
    'Get the first registry instance
    Set objRegSet = objService.InstancesOf("Win32_Registry")
    If blnErrorOccurred (" getting registry instance failed.") Then
        Exit Sub
    End If
    ...
    Call WriteLine("Modifying registry's proposed size on " & _
                    strServer, objOutputFile)
    objReg.ProposedSize = intSize
    Call objReg.Put_()
    If blnErrorOccurred (" modifying registry proposed size failed.") Then
        Exit Sub
    End If
...
Private Function intParseCmdLine( ByRef strServer,        _
    ...
    Do While intArgIter <= Wscript.arguments.Count - 1
        Select Case Left(LCase(Wscript.arguments.Item(intArgIter)),2)
            ...
            Case "/p"
                intParseCmdLine = CONST_PSIZE
                If Not blnGetArg("Proposed Size", intSize, intArgIter) Then
                    intParseCmdLine = CONST_ERROR
                    Exit Function
                End If
                intArgIter = intArgIter + 1
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Outputs or modifies the registry configuration."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  RegConfig.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>]"
    Wscript.Echo "  [/O <outputfile>] [/P <proposedsize>]"
    Wscript.Echo ""
    Wscript.Echo "PARAMETER SPECIFIERS:"
    Wscript.Echo "   proposedsize  New proposed size for registry."
    ...
    Wscript.Echo "1. cscript RegConfig.vbs"
    Wscript.Echo "   Lists the registry configuration on the current machine."
    Wscript.Echo "2. cscript RegConfig.vbs /S MyMachine2"
    Wscript.Echo "   Lists the registry configuration on machine MyMachine2."
    Wscript.Echo "3. cscript RegConfig.vbs /P 40"
    Wscript.Echo "   Changes the proposed size to 40 MB on the current machine."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript RegConfig.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""RegConfig.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
