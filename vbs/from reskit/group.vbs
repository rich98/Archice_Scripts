...
'* Version:        1.0
'*
'*  Main Function:  Outputs the groups in the specified domain.
'*
'*  Group.vbs      [/S <server>] [/U <username>] [/W <password>] 
'*                 [/O <outputfile>] [/D <domain>]
'*
...
OPTION EXPLICIT
    ...
    'Declare variables
    Dim intOpMode, i
    Dim strServer, strUserName, strPassword, strOutputFile, strDomain
...
Private Sub LstGroups(strServer     , _
    ...
    Dim objService, objGroupSet, objGroup, objWshNet
    Dim strWBEMClass

    strWBEMClass = "Win32_Group"

    'Get the server's name (i.e. local or remote domain) if left unspecified
    If IsEmpty(strDomain) Then
        If IsEmpty(strServer) Then
            Set objWshNet = CreateObject("Wscript.Network")
            strDomain = objWshNet.ComputerName
        Else
            strDomain = strServer
        End If
    End If
    ...
    'Get the set
    Set objGroupSet = objService.ExecQuery("Select * from " & _
                      strWBEMClass & " where Domain = """ & strDomain & """",,0)
    If blnErrorOccurred("Could not obtain " & strWBEMClass & " instance.") Then
        Exit Sub
    End If

    If objGroupSet.Count = 0 Then
        Call WriteLine("No group information is available.", objOutputFile)    
        Exit Sub
    End If

    Call WriteLine("Account groups for domain "& strDomain, objOutputFile)
    Call WriteLine("", objOutputFile)

    For Each objGroup In objGroupSet
        Call WriteLine("Name   = " & objGroup.Name, objOutputFile)
        Call WriteLine("Status = " & objGroup.Status, objOutputFile)
        Call WriteLine("", objOutputFile)
    Next
...
Private Function intParseCmdLine( ByRef strServer     ,  _
    ...
    Do While intArgIter <= Wscript.arguments.Count - 1
        Select Case Left(LCase(Wscript.arguments.Item(intArgIter)),2)
            ...
            Case "/d"
                If Not blnGetArg("Domain", strDomain, intArgIter) Then
                    intParseCmdLine = CONST_ERROR
                    Exit Function
                End If
                intArgIter = intArgIter + 1
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Outputs the groups in the specified domain."
    Wscript.Echo ""
    Wscript.Echo "SYNTAX:"
    Wscript.Echo "  Group.vbs [/S <server>] [/U <username>]" _
                &" [/W <password>] [/D <domain>]"
    Wscript.Echo "  [/O <outputfile>]"
    Wscript.Echo ""
    Wscript.Echo "PARAMETER SPECIFIERS:"
    Wscript.Echo "   domain        Desired domain for group listing."
    ...
    Wscript.Echo "1. cscript Group.vbs"
    Wscript.Echo "   Lists the account groups for the current machine."
    Wscript.Echo "2. cscript Group.vbs /S MyMachine2"
    Wscript.Echo "   Lists the account groups for the machine MyMachine2."
...
Sub VerifyHostIsCscript()
    ...
    If intStatus <> CONST_CSCRIPT Then
        Call WScript.Echo( "Please run this script using CScript." & vbCRLF & _
             ...
             "1. Using ""CScript Group.vbs arguments"" for Windows 95/98 or" _
             ...
             "    ""Group.vbs arguments"" for Windows NT/2000." )
        WScript.Quit
    End If
